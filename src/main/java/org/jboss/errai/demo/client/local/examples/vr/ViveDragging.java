...
public class ViveDragging extends Attachable {
    ...
    public ViveDragging() {
        ...
        controller1.addEventListener("triggerdown", (sender, event) -> onTriggerDown(event));
        controller1.addEventListener("triggerup", (sender, event) -> onTriggerUp(event));

        scene.add(controller1);
        controller2 = new ViveController(1);

        controller2.standingMatrix = webGLRenderer.vr.getStandingMatrix();
        controller2.addEventListener("triggerdown", (sender, event) -> onTriggerDown(event));
        controller2.addEventListener("triggerup", (sender, event) -> onTriggerUp(event));
    ...
    private void onTriggerDown(Object event) {
        ViveController controller = Js.uncheckedCast(Js.asPropertyMap(event).get("target"));
        ...
        if (intersections.length > 0) {
            ...
            controller.userData.set("selected", object);
        }
    }

    public void onTriggerUp(Object event) {
        ViveController controller = Js.uncheckedCast(Js.asPropertyMap(event).get("target"));

        if (controller.userData.has("selected")) {
            Mesh object = (Mesh) controller.userData.get("selected");

            object.matrix.premultiply(controller.matrixWorld);
            object.matrix.decompose(object.position, object.quaternion, object.scale);
            ((MeshStandardMaterial) object.material).emissive.b = 0;
            group.add(object);
            controller.userData.delete("selected");
        }
    }
    ...
    public void intersectObjects(ViveController controller) {
        ...
        if (!controller.userData.has("selected")) return;
