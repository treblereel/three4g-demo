...
public class WebglMaterialsDisplacementmap extends Attachable {
    ...
    public WebglMaterialsDisplacementmap() {
        ...
        scene = new Scene();
        camera = new OrthographicCamera((float) -getHeight() * aspect, (float) getHeight() * aspect, (float) getHeight(), (float) -getHeight(), 1, 10000);
        ...
        parameters.normalScale = new Vector2(1, -1); // why does the normal map require negation in this case?
    ...
    private void initGui() {
        ...
        gui.add("metalness", metalness, 0, 1).setStep(0.1).onChange(result -> material.metalness = result.floatValue());
        gui.add("roughness", roughness, 0, 1).setStep(0.1).onChange(result -> material.roughness = result.floatValue());
        gui.add("aoMapIntensity", aoMapIntensity, 0, 1).setStep(0.1).onChange(result -> material.aoMapIntensity = result.floatValue());
        gui.add("ambientIntensity", ambientIntensity, 0, 1).setStep(0.1).onChange(result -> ambientLight.intensity = result.floatValue());
        gui.add("envMapIntensity", envMapIntensity, 0, 3).setStep(0.1).onChange(result -> material.envMapIntensity = result.floatValue());
        gui.add("displacementScale", envMapIntensity, 0, 3).setStep(0.1).onChange(result -> material.displacementScale = result.floatValue());
        gui.add("normalScale", envMapIntensity, 0, 3).setStep(0.1).onChange(result -> material.normalScale.set(1, -1).multiplyScalar(result.floatValue()));
    ...
    protected void doAttachInfo() {
        AppSetup.infoDiv.show().setHrefToInfo("http=//threejs.org").setTextContentToInfo("three.js").setInnetHtml("- (<span id=\"description\">normal + ao + displacement + environment</span>) map demo.<br />\n" +
